 ...
 * VERSION: 4.0 — Redirects to /result/[id] after prediction
 * SIGNED: ROHIIT GUPTA, CEO
 *
 * FIXES IN v4.0:
 *   - After prediction success, reads _meta.predictionId
 *   - Redirects to /result/[predictionId] (server-rendered page)
 *   - Removes old inline result display
 *   - Generates proper sessionId with tv_ prefix (matches route.ts)
 *   - Passes language, segment from form context
 * ============================================================
 */
...
export default function BirthForm({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const predictionId = data?._meta?.predictionId
      ...
      if (predictionId) {
        ...
        // Fallback: predictionId missing (Supabase save may have failed)
        // Use sessionId-based URL so at least something shows
        setApiError(
          "Prediction ready hai par save nahi hua. Please retry karo."
        )
        setStatusMsg("")
        console.error("[BirthForm] predictionId missing in response:", data?._meta)
      }
        ...
        {selectedCategory && (
          <div className="mb-6 text-center">
            ...
            <p className="text-slate-400 text-sm">
              Enter your birth details for a personalized reading
            </p>
          </div>
        )}
        ...
        >
          <form onSubmit={handleSubmit} noValidate className="grid gap-5">
            ...
            <div className="relative">
              ...
              {errors.latitude && (
                <p className="text-red-400 text-xs mt-1">Please select a place from suggestions</p>
              )}
            </div>
            ...
            <input type="hidden" value="lahiri" />

            {/* Status message */}
            {statusMsg && (
              <div
                className="px-4 py-3 rounded-lg text-sm text-yellow-300 text-center"
            ...
            >
              {isLoading ? statusMsg || "Calculating your chart…" : submitLabel}
            </button>
