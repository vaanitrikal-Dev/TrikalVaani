 ...
 * VERSION: 5.3 — Language selector + Job categories + Mobile + Numerology
...
function getNumerologyCompatibility(n1: number, n2: number): {
  ...
  const COMPATIBILITY: Record<string, { score: number; label: string; description: string; color: string }> = {
  ...
  return COMPATIBILITY[key1] ?? {
    score: 65,
    label: 'Moderate',
    description: 'Unique combination — balance and understanding is key.',
    color: '#f59e0b',
  }
}
...
function offsetFromLon(lon: number): number {
  if (lon >= 68 && lon <= 97) return 5.5   // India → IST
  if (lon >= 80 && lon <= 88) return 5.75  // Nepal
...
export default function BirthForm({
  ...
  // ── Mobile numerology live calculation ───────────────────────────────────
  const handleMobileChange = (val: string, isP2 = false) => {
    ...
    if (isDualDomain && m1.replace(/\D/g,'').length >= 10 && m2.replace(/\D/g,'').length >= 10) {
      const n1 = getMobileNumber(m1)
      const n2 = getMobileNumber(m2)
      setNumerology(getNumerologyCompatibility(n1, n2))
    } else {
      setNumerology(null)
    }
  }
  ...
  const validate = (): boolean => {
    ...
    if (!fields.unknownTime && !fields.timeOfBirth) errs.timeOfBirth = "Time required"
    if (fields.latitude === '')     errs.latitude = "Place of birth is required"
    if (!fields.mobile || fields.mobile.replace(/\D/g,'').length < 10)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      try { sessionStorage.setItem('tv_last_prediction', JSON.stringify(payload)) } catch { }
        ...
        <div className="rounded-2xl p-6 sm:p-8"
          ...
          <form onSubmit={handleSubmit} noValidate className="grid gap-5">
              ...
              <div className="grid grid-cols-3 gap-2">
                {LANGUAGE_OPTIONS.map(opt => (
                  <button key={opt.value} type="button"
                    ...
                    style={{
                      background: fields.language === opt.value ? 'rgba(212,175,55,0.2)' : 'rgba(255,255,255,0.04)',
                      border: `1px solid ${fields.language === opt.value ? 'rgba(212,175,55,0.6)' : 'rgba(255,255,255,0.1)'}`,
                      color: fields.language === opt.value ? '#D4AF37' : '#94a3b8',
                    }}>
            ...
            <div className="relative">
              ...
              <div className="relative">
                <input id="tv-place" type="search" autoComplete="off"
                ...
                {geoLoading && <span className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 text-xs">⟳</span>}
              </div>
              {geoResults.length > 0 && (
            ...
            {/* ── PERSON 2 SECTION (dual chart domains only) ── */}
            {isDualDomain && (
            ...
            <button type="submit" disabled={isLoading}
              ...
              {isLoading ? "Jini pad rahi hai aapki kundali…" : submitLabel}
            </button>
