 ...
 * File: components/jini/TrikalVoice.tsx
 * VERSION: 1.1 — Renamed JiniVoice → TrikalVoice
 * SIGNED: ROHIIT GUPTA, CEO
 *
 * FEATURES:
 *   → Floating RIGHT side widget (separate from TrikalChat)
 ...
 *   → "Trikal sun raha hai..." live animation during recording
...
const TAGLINES = [
  ...
  'Bol do Trikal ko — woh sun raha hai',
  'Dil ki baat — sirf ek minute mein',
  'Trikal sun raha hai — bas shuru karo',
  'Jo dil mein hai — woh bol do',
  'Apni awaaz, apni kahani — Trikal ke saath',
  'Type mat karo — feel karo',
  'Ek minute — aur Trikal samjh jayega',
];
...
function CosmicMicIcon({ isRecording, size = 28 }: { isRecording: boolean; size?: number }) {
  return (
    <svg width={size} height={size} viewBox="0 0 28 28" fill="none" xmlns="http://www.w3.org/2000/svg">
      <rect x="9" y="2" width="10" height="16" rx="5" fill={isRecording ? '#ef4444' : GOLD} style={{ transition: 'fill 0.3s ease' }} />
      ...
      <path d="M6 14 C6 20 22 20 22 14" stroke={isRecording ? '#ef4444' : GOLD} strokeWidth="2" strokeLinecap="round" fill="none" style={{ transition: 'stroke 0.3s ease' }} />
...
export default function TrikalVoice({ onVoiceSubmit, language = 'hinglish' }: TrikalVoiceProps) {
  ...
  const startRecording = useCallback(async () => {
    try {
      ...
      mediaRecorder.onstop = async () => {
        ...
        setTimeout(() => {
          const mockTranscript = 'Voice input captured — Trikal AI processing...';
  ...
  const TriggerButton = () => (
    <button
      ...
      aria-label="Trikal Vaani Voice — Apni baat boliye"
      title="Trikal se baat karo 🎙️"
      style={{
        position:       'fixed',
        right:          '20px',
    ...
    >
      {pulseRing && (
        <div style={{
          ...
          animation: 'trikalPulseRing 1s ease-out forwards', pointerEvents: 'none',
        }} />
      )}
      <CosmicMicIcon isRecording={false} size={28} />
      <style>{`
        @keyframes trikalPulseRing {
  ...
  const VoicePanel = () => (
    <div style={{
      ...
      animation:      'trikalSlideUp 0.3s ease-out',
    }}>
      <style>{`
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
          <div style={{
            ...
            fontSize:       '16px',
            animation:      isRecording ? 'trikalBreathe 1s ease-in-out infinite' : 'none',
          }}>
            🔮
          </div>
          <div>
            ...
            <p style={{ margin: 0, color: `rgba(212,175,55,0.7)`, fontSize: '10px', fontWeight: 500, letterSpacing: '0.05em' }}>
              {isRecording ? 'SUN RAHA HAI... 🎙️' : isProcessing ? 'SAMAJH RAHA HAI...' : isDone ? 'HO GAYA ✓' : 'TRIKAL VAANI'}
            </p>
          </div>
        </div>
        <button
      ...
      <div style={{ padding: '24px 20px' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '20px', marginBottom: '24px' }}>
          ...
          {!isDone && !isProcessing && (
            ...
            >
              {isRecording && (
                <>
                  <div style={{ position: 'absolute', width: '100%', height: '100%', borderRadius: '50%', border: '2px solid rgba(220,38,38,0.4)', animation: 'trikalPulseRing 1.5s ease-out infinite' }} />
                  <div style={{ position: 'absolute', width: '100%', height: '100%', borderRadius: '50%', border: '2px solid rgba(220,38,38,0.2)', animation: 'trikalPulseRing 1.5s ease-out infinite 0.5s' }} />
                </>
              )}
          ...
          {isProcessing && (
            <div style={{ width: '100px', height: '100px', borderRadius: '50%', background: `radial-gradient(circle, rgba(212,175,55,0.1) 0%, transparent 70%)`, display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative' }}>
              <div style={{ position: 'absolute', width: '90px', height: '90px', borderRadius: '50%', border: `2px solid transparent`, borderTop: `2px solid ${GOLD}`, animation: 'trikalSpin 1s linear infinite' }} />
              <span style={{ fontSize: '32px' }}>🔮</span>
            </div>
          )}

          {isDone && (
            <div style={{ width: '100px', height: '100px', borderRadius: '50%', background: 'radial-gradient(circle, rgba(34,197,94,0.15) 0%, transparent 70%)', display: 'flex', alignItems: 'center', justifyContent: 'center', border: '2px solid rgba(34,197,94,0.3)', boxShadow: '0 0 30px rgba(34,197,94,0.2)' }}>
          ...
          {isRecording && (
            <div style={{ textAlign: 'center' }}>
              <p style={{ color: countdown <= 10 ? '#ef4444' : GOLD, fontSize: '28px', fontWeight: 800, margin: 0, fontFamily: 'Georgia, serif', transition: 'color 0.3s' }}>
                {countdown}s
              </p>
              <p style={{ color: '#64748b', fontSize: '11px', margin: '4px 0 0' }}>
                baat karo — Trikal sun raha hai
              </p>
            </div>
          )}
          ...
          {isProcessing && (
            <p style={{ color: GOLD, fontSize: '13px', margin: 0, textAlign: 'center', fontStyle: 'italic' }}>
              Trikal aapki baat samajh raha hai...
            </p>
          )}
          ...
          {isDone && (
            <div style={{ textAlign: 'center', width: '100%' }}>
              ...
              {transcript && (
                <div style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '10px', padding: '10px 12px', marginBottom: '12px' }}>
                  <p style={{ color: '#94a3b8', fontSize: '11px', margin: 0, lineHeight: 1.5 }}>{transcript}</p>
                </div>
              )}
            </div>
          )}
        ...
        <div style={{ display: 'flex', gap: '10px' }}>
          {isRecording && (
            <button onClick={stopRecording} style={{ flex: 1, padding: '12px', background: 'rgba(239,68,68,0.1)', border: '1px solid rgba(239,68,68,0.3)', borderRadius: '12px', color: '#ef4444', fontSize: '13px', fontWeight: 600, cursor: 'pointer' }}>
              ⏹ Roko
            </button>
          )}
          {isDone && (
            <>
              <button onClick={reset} style={{ flex: 1, padding: '12px', background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '12px', color: '#94a3b8', fontSize: '13px', cursor: 'pointer' }}>
                🔄 Dobara
              </button>
              <button style={{ flex: 2, padding: '12px', background: `linear-gradient(135deg, ${GOLD_DARK}, ${GOLD}, ${GOLD_LIGHT})`, border: 'none', borderRadius: '12px', color: BG_DARK, fontSize: '13px', fontWeight: 700, cursor: 'pointer' }}>
                🔮 Trikal se jawab lo
              </button>
            </>
          )}
        ...
        <div style={{ marginTop: '20px', paddingTop: '16px', borderTop: '1px solid rgba(255,255,255,0.06)', textAlign: 'center' }}>
          <p style={{ color: `rgba(212,175,55,0.6)`, fontSize: '11px', margin: 0, lineHeight: 1.5 }}>
