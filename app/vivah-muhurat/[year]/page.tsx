// ============================================================
//  Trikaal Vaani — Vivah Muhurat [year] (Shubh Marriage Dates)
...
interface Muhurat {
  date: string; weekday: string; nakshatra: string; tithi: string;
...
export async function generateMetadata(
  ...
  return {
    ...
    openGraph: {
      title: `Vivah Muhurat ${year} — Shubh Marriage Dates | Trikaal Vaani`,
      description: `Strict-classical Vivah Muhurat ${year} dates with exact time, nakshatra, tithi and lagna. Parashara BPHS based.`,
      url: `${SITE}/vivah-muhurat/${year}`,
      type: 'article',
...
function crossesMidnight(s: string, e: string) { return /PM/i.test(s) && /AM/i.test(e); }
...
export default async function VivahMuhuratYearPage(
  { params }: { params: { year: string } },
) {
  ...
  const lastLong = total ? fmtLong(muhurats[total - 1].date) : '';

  const faqs = [
    {
      q: `What are the best marriage (vivah) dates in ${year}?`,
      a: total
        ? `In ${year} there are ${total} strictly auspicious vivah muhurats, running from ${firstLong} to ${lastLong}. Marriage pauses during ${pausedList}, resuming after Devuthani (Prabodhini) Ekadashi.`
        : `Vivah muhurat dates for ${year} are computed by strict Parashara rules, excluding ${pausedList}.`,
    },
    {
      q: `Why are some months in ${year} empty of vivah dates?`,
      a: 'Those months fall inside Chaturmas — the four-month sleep period of Lord Vishnu (Devshayani to Devuthani Ekadashi). By classical Sanatan tradition no Hindu marriage is solemnised during Chaturmas, so Trikaal Vaani shows zero vivah dates in that window.',
    },
    {
      q: 'What is Kharmas and why are those dates excluded?',
      a: 'Kharmas (Malmas) is the period when the Sun transits Dhanu (Sagittarius) or Meen (Pisces). All auspicious sanskars including vivah are paused during Kharmas.',
    },
    {
  ...
  const jsonLd = {
    ...
    '@graph': [
      ...
      {
        '@type': 'Person', '@id': `${SITE}/#rohiit`,
        name: 'Rohiit Gupta', jobTitle: 'Chief Vedic Architect', worksFor: { '@id': `${SITE}/#organization` },
        knowsAbout: ['Vedic Astrology', 'Jyotish', 'Muhurat', 'Parashara BPHS', 'Vivah Muhurat'],
      },
      {
        '@type': 'WebPage', '@id': `${SITE}/vivah-muhurat/${year}`, url: `${SITE}/vivah-muhurat/${year}`,
        name: `Vivah Muhurat ${year} — Shubh Marriage Dates with Time, Nakshatra & Lagna`,
        isPartOf: { '@id': `${SITE}/#website` }, about: { '@id': `${SITE}/#organization` },
        author: { '@id': `${SITE}/#rohiit` }, publisher: { '@id': `${SITE}/#organization` }, inLanguage: 'en-IN',
        description: `Strict-classical Vivah Muhurat ${year} dates with exact time, nakshatra, tithi and lagna, computed by Parashara BPHS rules.`,
      },
      { '@type': 'WebSite', '@id': `${SITE}/#website`, url: SITE, name: 'Trikaal Vaani', publisher: { '@id': `${SITE}/#organization` } },
      {
        '@type': 'BreadcrumbList',
        itemListElement: [
          { '@type': 'ListItem', position: 1, name: 'Home', item: SITE },
          { '@type': 'ListItem', position: 2, name: 'Vivah Muhurat', item: `${SITE}/vivah-muhurat` },
          { '@type': 'ListItem', position: 3, name: `${year}`, item: `${SITE}/vivah-muhurat/${year}` },
        ],
      },
      {
        '@type': 'FAQPage',
      ...
      <div style={{ maxWidth: 1080, margin: '0 auto', padding: '32px 20px 80px' }}>
        ...
        {allYears.length > 1 && (
          <div style={{ display: 'flex', gap: 10, justifyContent: 'center', marginBottom: 34, flexWrap: 'wrap' }}>
            {allYears.map((y) => (
              <Link key={y} href={`/vivah-muhurat/${y}`}
                style={{
                  padding: '8px 20px', borderRadius: 999, textDecoration: 'none', fontSize: 14, fontWeight: 600,
                  background: y === year ? GOLD : GOLD_RGBA(0.06),
                  color: y === year ? '#1A1206' : GOLD,
                  border: `1px solid ${GOLD_RGBA(y === year ? 0.9 : 0.25)}`,
                }}>
                {y}
              </Link>
            ))}
        ...
        <section className="tv-aeo-answer"
          ...
          <p style={{ margin: 0, fontSize: 16.5, lineHeight: 1.7, color: '#E2E8F0' }}>
            {total ? (
              <>
                The most auspicious <strong style={{ color: GOLD }}>Vivah Muhurat {year}</strong> dates run from
                <strong> {firstLong}</strong> to <strong>{lastLong}</strong>, with marriage paused during <strong>{pausedList}</strong>.
                Trikaal Vaani lists <strong style={{ color: GOLD }}>{total} strictly shubh muhurats</strong> for {year} — each computed by
                Parashara BPHS rules (auspicious nakshatra, tithi, vaar, Bhadra-free window) with exact muhurat timing and lagna.
              </>
            ) : (
              <>
                <strong style={{ color: GOLD }}>Vivah Muhurat {year}</strong> dates are computed by strict Parashara BPHS rules —
                auspicious nakshatra, tithi, vaar and a Bhadra-free window — excluding <strong>{pausedList}</strong>. Dates are being prepared.
              </>
            )}
          </p>
        </section>
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', gap: 7, fontSize: 13.5 }}>
                        ...
                        <Row label="Lagna">
                          <span style={{ color: '#E2E8F0' }}>
                            {m.lagna}{' '}
                            <span style={{ color: qualityColor(m.lagna_quality), fontSize: 11.5 }}>• {m.lagna_quality}</span>
                          </span>
                        </Row>
                      </div>
                    </div>
        ...
        {windows.length > 0 && (
          <section style={{ marginBottom: 48 }}>
            <h2 style={{ fontFamily: 'Georgia, serif', fontSize: 26, color: '#F8FAFC', marginBottom: 8 }}>
              Forbidden Periods for Vivah in {year}
            </h2>
        ...
        <article style={{ fontSize: 15.5, lineHeight: 1.8, color: '#CBD5E1', marginBottom: 48 }}>
          ...
          <p style={{ marginBottom: 18 }}>
            ...
            vivah shapes the harmony, longevity and prosperity of the union — so families plan weddings, venues and bookings
            around these dates months in advance.
          </p>
          <p style={{ marginBottom: 18 }}>
            A genuine muhurat must satisfy several conditions at once: an auspicious <strong style={{ color: '#F8FAFC' }}>nakshatra</strong>,
          ...
          <p style={{ marginBottom: 18 }}>
            ...
            auspicious nakshatra and tithi do not actually overlap during clean hours — a deliberately smaller,
            <strong style={{ color: GOLD }}> defensible</strong> set a careful family astrologer would actually approve.
          </p>
        </article>
