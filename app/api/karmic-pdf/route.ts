/**
 * ============================================================
 * TRIKAL VAANI — Karmic PDF Proxy API
 * CEO & Chief Vedic Architect: Rohiit Gupta
 * File: app/api/karmic-pdf/route.ts
 * VERSION: 1.1
 * ============================================================
 * On-demand PDF generation for paid Karmic Background Readings.
 * Mirrors app/api/muhurat-pdf/route.ts exactly.
 *
 ...
 *   2. Verify karmic_readings record exists (anti-tamper)
 *   3. Verify linked order is paid
 *   4. Verify narrative exists (PDF needs content)
 *   5. Call VM /karmic-pdf with slug
...
const VM_KARMIC_PDF_ENDPOINT =
  process.env.VM_KARMIC_PDF_ENDPOINT ?? 'http://34.14.164.105:8001/karmic-pdf';
...
export async function POST(req: NextRequest) {
  try {
    ...
    // Anti-tamper: verify reading exists
    ...
    if (loadErr || !reading) {
      console.error('[Trikal] Karmic PDF: reading not found:', slug, loadErr?.message);
    ...
    // Idempotency: return cached url if present
    if (reading.pdf_url && reading.pdf_url.startsWith('https://')) {
      return NextResponse.json({
        success: true, slug, pdf_url: reading.pdf_url, cached: true,
      });
    }
    ...
    let vmRes: Response;
    try {
      vmRes = await callVM(VM_KARMIC_PDF_ENDPOINT, {
        method:  'POST',
      ...
      console.error('[Trikal] VM /karmic-pdf fetch failed:', e);
      return NextResponse.json(
    ...
    if (!vmRes.ok) {
      ...
      console.error('[Trikal] VM /karmic-pdf error:', vmRes.status, txt);
      return NextResponse.json(
    ...
    console.error('[Trikal] /api/karmic-pdf error:', err);
