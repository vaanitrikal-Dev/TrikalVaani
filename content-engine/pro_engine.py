...
WHAT CHANGED IN v3.2.0 (visual variety + SEO):
  ...
  - TITLE-CARD INTRO: the thumbnail is now prepended as a 1.2s freeze-frame at
    the start of every video — Instagram/Reels auto-picks it as the cover AND
    viewers see the topic title in the first second. Fail-safe (skips if the
    thumbnail fails; video still renders).
=============================================================
...
try:
    ...
    load_dotenv(Path("/home/vaanitrikal/trikal-vaani/content-engine/.env"), override=True)
...
def _finish_and_upload(script_data, images, audio, slug, pipeline,
    ...
    # --- THUMBNAIL (built EARLY so it can also become the intro frame) ---
    thumb_path = None
    ...
    # v3.2: TITLE CARD INTRO — freeze the thumbnail as a 1.2s silent clip at
    # the very start. This makes Instagram/Reels auto-pick it as the cover AND
    # shows viewers the topic title in the first second. Fail-safe.
    intro_clip = None
    if thumb_path and thumb_path.exists():
        try:
            intro_clip = TEMP_DIR / f"pro_intro_{slug}.mp4"
            subprocess.run([
            ...
            if intro_clip.exists() and intro_clip.stat().st_size > 1000:
                log("  Title-card intro built (1.2s thumbnail freeze)")
            else:
                intro_clip = None
                log("  Title-card intro skipped (render empty)")
        except Exception as e:
            intro_clip = None
            log(f"  Title-card intro skipped (non-fatal): {e}")

    # assemble: [title-card intro?] + [hook?] + slides + [end card]
    parts = []
