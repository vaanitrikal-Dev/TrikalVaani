 ...
 * ⚠️ STRICT CEO ORDER: LOGIC FROZEN
 * DO NOT EDIT, DELETE, OR REFACTOR THIS FILE.
 * VERSION: 15.0-FINAL (GOD-LEVEL PROTECTION)
 * SIGNED: ROHIIT GUPTA, CEO
 * PURPOSE: UNIFIED JINI API — SWISS EPHEMERIS + GEMINI + JINI ENGINE
 * WARNING: DO NOT CHANGE MODEL NAME, SYSTEM_INSTRUCTION STRUCTURE, OR ERROR HANDLING
 */
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!GEMINI_API_KEY) {
      return NextResponse.json({ reply: 'API Key missing — Vercel mein set karein.' });
    }

    // First message — always return Jini's signature opening
    if (isFirstMessage) {
      return NextResponse.json({ reply: JINI_NAMASTE, kundaliSummary: null });
    }

    // Build Kundali if birth data provided
    let kundali = null;
    ...
    if (birthData?.dob && birthData?.tob && birthData?.lat) {
      try {
        ...
        console.error('[Trikal Engine] Kundali calculation error:', calcErr);
      }
    }
    ...
    const res = await fetch(`${GEMINI_URL}?key=${GEMINI_API_KEY}`, {
      ...
      body: JSON.stringify({
        ...
        generationConfig: { temperature: 0.82, maxOutputTokens: 1500, topP: 0.9 },
      }),
    });

    if (!res.ok) {
      console.error(`[Trikal] Gemini HTTP error: ${res.status}`);
      return NextResponse.json({
        reply: 'Cosmic signals thodi der ke liye weak hain. Ek minute mein dobara try karein. 🙏',
        kundaliSummary,
      });
    }
    ...
    if (data.error) {
      console.error('[Trikal] Gemini API error:', data.error);
      return NextResponse.json({
        reply: 'Jini abhi meditate kar rahi hai. Thodi der mein aayegi. 🙏',
        kundaliSummary,
      });
    }

    const reply = data?.candidates?.[0]?.content?.parts?.[0]?.text?.trim()
      ?? 'Bhai, cosmic signals connect nahi hue. Dobara puchiye. 🙏';

    return NextResponse.json({ reply, kundaliSummary });

  } catch (err: unknown) {
    console.error('[Trikal] Unexpected error:', err);
    return NextResponse.json({
